...
def run_tdd():
    ...
    with cd(os.path.join( config.get('vm', 'base_dir') ,'wirefax')):
...
def reload(event):
    ...
    if run_tdd():
        local("osascript -e 'display notification \"wirefax unit tests FAILED!.\" with title \"Bummer\"'")    
        return
    sudo('supervisorctl restart wirefax')
    local("osascript -e 'display notification \"wirefax reloaded.\" with title \"Matrix Reloaded\"'")
...
def run():
    ...
    try:
        ...
        observer = Observer()
        watch = observer.schedule(event_handler, './wirefax', recursive=True)
