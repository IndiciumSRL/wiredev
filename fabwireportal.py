...
def run_tdd():
    ...
    with cd(os.path.join( config.get('vm', 'base_dir') ,'wireportalbackend')):
...
def reload(event):
    ...
    if run_tdd():
        with settings(warn_only=True):
            local("osascript -e 'display notification \"WirePortal unit tests FAILED!.\" with title \"Bummer\"'")    
            return
    sudo('supervisorctl restart wiremonitor')
    with settings(warn_only=True):
        local("osascript -e 'display notification \"WirePortal reloaded.\" with title \"Matrix Reloaded\"'")
